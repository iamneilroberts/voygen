...
export const TOOL_NAMES = {
  BROWSER: {
    ...
    PARSE_TRAVEL_FACTS: 'chrome_parse_travel_facts',
  },
  ORCHESTRATOR: {
    EXTRACT_HOTELS: 'extract_hotels',
    EXTRACT_ROOM_RATES: 'extract_room_rates',
  },
};
...
export const TOOL_SCHEMAS: Tool[] = [
  ...
  },
  {
    ...
    description:
      'Extract hotel search results from the current page (WAD/VAX/Navitrip/generic) via a single injected script. Returns gzipped NDJSON + sample.',
    inputSchema: {
      type: 'object',
      properties: {
        url: { type: 'string', description: 'Optional URL to navigate before extracting' },
        pageTypeHint: {
          type: 'string',
          enum: ['vax', 'wad', 'navitrip_cp', 'generic'],
          description: 'Optional page type hint to skip first-glance detection',
        },
        maxRows: { type: 'number', description: 'Max results to extract (default 5000)' },
        domSelector: { type: 'string', description: 'Custom DOM selector fallback' },
      },
      required: [],
    },
  },
  {
    ...
    description:
      'Parse ad-hoc travel content (invoices/confirmations/events/markets) from the current page. Returns gzipped JSON facts + sample.',
    inputSchema: {
      type: 'object',
      properties: {
        url: { type: 'string', description: 'Optional URL to navigate before parsing' },
        hint: { type: 'string', description: 'Free-form hint to bias parser (e.g., "hotel confirmation")' },
        maxChars: { type: 'number', description: 'Character budget for text scan (default 250k)' },
        preferKind: {
          type: 'array',
          description: 'Preferred kinds for targeting extraction',
          items: { type: 'string', enum: ['flight', 'hotel', 'reservation', 'event', 'place', 'generic'] },
        },
      },
      required: [],
    },
  },
  {
    ...
    description: 'Orchestrate hotel extraction and ingest into database; wraps chrome_extract_hotels',
    inputSchema: {
      type: 'object',
      properties: {
        trip_id: { type: 'string' },
        site: { type: 'string', enum: ['navitrip', 'trisept', 'vax', 'generic'] },
        search_params: {
          type: 'object',
          properties: {
        ...
        options: {
          type: 'object',
          properties: {
            max_hotels: { type: 'number' },
            price_range: {
              type: 'object',
              properties: { min: { type: 'number' }, max: { type: 'number' } },
            },
            star_rating: { type: 'number' },
            fetch_rooms: { type: 'boolean' },
            url: { type: 'string' },
          },
        },
      },
      required: ['trip_id', 'site', 'search_params'],
    },
  },
  {
    name: TOOL_NAMES.ORCHESTRATOR.EXTRACT_ROOM_RATES,
    description: 'Extract detailed room rates for given hotel IDs and ingest',
    inputSchema: {
      type: 'object',
      properties: {
        trip_id: { type: 'string' },
      ...
      required: ['trip_id', 'site', 'hotel_ids', 'search_params'],
    },
  },
];
