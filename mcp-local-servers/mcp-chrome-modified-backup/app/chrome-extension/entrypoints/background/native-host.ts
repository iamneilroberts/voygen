...
export function connectNativeHost(port: number = NATIVE_HOST.DEFAULT_PORT) {
  ...
  try {
    console.log(`[Chrome MCP] Connecting native host ${HOST_NAME} on ${port}...`);
    ...
    nativePort.onMessage.addListener(async (message) => {
      ...
      if (message.type === NativeMessageType.PROCESS_DATA && message.requestId) {
        ...
        console.log(`[Chrome MCP] ${SUCCESS_MESSAGES.SERVER_STARTED} on port ${port}`);
        ...
        await saveServerStatus(currentServerStatus);
        broadcastServerStatusChange(currentServerStatus);
        console.log(`[Chrome MCP] ${SUCCESS_MESSAGES.SERVER_STOPPED}`);
    ...
    nativePort.onDisconnect.addListener(() => {
      console.error('[Chrome MCP] ' + ERROR_MESSAGES.NATIVE_DISCONNECTED, chrome.runtime.lastError);
      nativePort = null;
    });

    nativePort.postMessage({ type: NativeMessageType.START, payload: { port } });
    console.log(`[Chrome MCP] Sent START to native host on ${port}`);
  } catch (error) {
    console.error('[Chrome MCP] ' + ERROR_MESSAGES.NATIVE_CONNECTION_FAILED, error);
  }
}
...
export const initNativeHostListener = () => {
  ...
  // Attempt immediate connection on background init
  connectNativeHost(NATIVE_HOST.DEFAULT_PORT);
  // And also connect on Chrome startup for resiliency
