...
export class Server {
  ...
  public async start(port = NATIVE_SERVER_PORT, nativeHost: NativeMessagingHost): Promise<void> {
    ...
    try {
      ...
      const uri = process.env.MONGODB_URI ? 'present' : 'missing';
      // IMPORTANT: Never write to stdout in a Native Messaging host process.
      // Use stderr for diagnostics; stdout is reserved for length‑framed messages.
      console.error(`[MCP HTTP] Listening on http://${SERVER_CONFIG.HOST}:${port} (MONGODB_URI: ${uri})`);
