...
export async function createManifestContent(): Promise<any> {
  ...
  const idsEnv = process.env.CHROME_EXTENSION_IDS || process.env.CHROME_EXTENSION_ID || '';
  const ids = (idsEnv ? String(idsEnv) : EXTENSION_ID)
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  const uniqIds = Array.from(new Set(ids.length ? ids : [EXTENSION_ID]));
  ...
  return {
    ...
    type: 'stdio',
    allowed_origins: uniqIds.map((id) => `chrome-extension://${id}/`),
  };
}
