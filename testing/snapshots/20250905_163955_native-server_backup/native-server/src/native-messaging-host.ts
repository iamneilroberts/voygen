    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'ping_from_extension':
          ...
          break;
        // Accept common synonyms from older/newer extensions
        case 'start':
        case 'starthost':
        case 'start_host':
        case 'startservice':
        case 'start_service':
        case 'service_start':
          await this.startServer(message.payload?.port || Number(process.env.MCP_HTTP_PORT) || 12306);
          break;
        default:
  ...
  private async startServer(port: number): Promise<void> {
    ...
    try {
      if (this.associatedServer.isRunning) {
        const runningPort = (this.associatedServer as any).getPort?.() ?? port;
        this.sendMessage({
          type: NativeMessageType.SERVER_STARTED,
          payload: { port: runningPort },
        });
        return;
      }
      ...
      if (error && (error.code === 'EADDRINUSE' || /EADDRINUSE/.test(String(error?.message)))) {
        // Treat as idempotent: someone already listening on the port
        const runningPort = (this.associatedServer as any).getPort?.() ?? port;
        this.sendMessage({
          type: NativeMessageType.SERVER_STARTED,
          payload: { port: runningPort },
      ...
      this.sendError(`Failed to start server: ${error?.message || String(error)}`);
    }
  }
