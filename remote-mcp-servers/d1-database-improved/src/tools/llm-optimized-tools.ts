...
import { limitQueryComplexity, sanitizeSearchTerm, assessQueryComplexity, monitorQueryPerformance } from '../utils/query-optimizer';
...
async function logActivity(
  ...
  try {
    const finalSessionId = sessionId || generateSessionId('Activity');
...
export const getAnythingTool = {
  name: 'get_anything',
  description: 'Get COMPLETE information about any trip, client, or travel-related query. Just describe what you need in natural language.',
  inputSchema: z.object({
    query: z.string().describe('Natural language query like "Smith family Hawaii trip" or "john@email.com" or "upcoming trips"'),
...
TRIPS: ${tripHistory.length > 0 ? tripHistory.map((t: any) => t.trip_name).join(', ') : 'No trips assigned'}`,
  ...
  Workflow: ${workflowStatus}`;
      ...
      if (complexity === 'complex') {
        ...
        // Extract meaningful terms (same logic as word search section)
        ...
        // Try simple direct search instead of complex view query
        try {
          let simpleResults = [];
          
          // Simple trip search with workflow state
          for (const term of searchTerms) {
            const tripResults = await executeWithTimeout(
              db.prepare(`
                ...
                LIMIT 3
              `).bind(`%${term}%`).all(),
              `simple trip search for complex query term "${term}"`
            );
          ...
          console.warn('Simple search for complex query failed, continuing with normal search:', e);
        }
      }
      ...
      // No results found
      return {
        response: `No results found for "${input.query}". Try searching by:\n` +
          `- Trip name (e.g., "European Adventure")\n` +
          `- Client name or email\n` +
          `- Keywords like "upcoming trips" or "total revenue"\n` +
          `- Destination names`,
        error: 'not_found',
        complexity,
        search_terms: limitQueryComplexity(searchTerm.split(/\s+/))
      } as SearchResponse;
    } catch (error: any) {
...
async function findTripSafely(db: D1Database, tripIdentifier: string): Promise<any> {
  try {
    ...
    // If no exact match and identifier looks like an ID, return null
    if (/^\d+$/.test(tripIdentifier)) {
      return null;
    }

    // For complex trip names, try word-based search to avoid LIKE pattern issues
    const words = tripIdentifier.toLowerCase().split(/\s+/).filter(w => w.length >= 3);
    if (words.length > 0) {
      // Use the first meaningful word in search_text column which is simpler
...
export const continueTripTool = {
  ...
  handler: async (input: any, db: D1Database) => {
    const sessionId = generateSessionId('continue');
