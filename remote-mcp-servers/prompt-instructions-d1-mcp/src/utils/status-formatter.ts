...
export function extractTripStatus(rawData: any, contextType: string): TripStatus {
  ...
  try {
    if (contextType === 'trip_direct' || contextType === 'trip') {
      // Parse workflow state if available
      if (rawData.workflow_state) {
        const workflow = typeof rawData.workflow_state === 'string' 
          ? JSON.parse(rawData.workflow_state) 
          : rawData.workflow_state;
      ...
      // Extract trip info
      status.tripName = rawData.trip_name || rawData.natural_key;
      ...
      if (rawData.total_cost) {
        status.budgetUsed = rawData.total_cost;
        status.budgetTotal = rawData.budget_limit || rawData.total_cost * 1.2; // Estimate if not set
      }
    }
  } catch (error) {
